...
int main(int argc,char *argv[])
{
	...
	rndr->CopyDraw(1,rndr->viewport, sten_viewport); //added display info 4
	rndr->ClearDrawFlag(sten_drawinfo, rndr->toClear);
	rndr->SetDrawFlag(sten_drawinfo, rndr->blend | rndr->inAction2 | rndr->scissorTest);
	rndr->AddDraw(sten_viewport, 0, 4, 0, rndr->stencil2 | rndr->stencilTest | rndr->depthTest | rndr->scaleAbit | rndr->onPicking);
